...
#_(defn update-post! [{:strs [title author date preview preview-img title-img tags content] :as params}]
...
#_(defn update-uploads! [{:strs [file]}]
  (if (and (> (:size file) 0)
...
;;tags
#_(defn contains-tag? [tag post]
  (if-let [tags (:tags post)]
    (contains? tags tag)))

#_(defn get-all-tags [posts]
  (->> posts
       (map :tags)
       (apply union)
       (remove nil?)))
...
#_(defn str->keyword [string]
  (keyword (clojure.string/replace string #"\s+" "-")))

#_(defn create-user! [{:keys [username password] :as user}]
...
#_(defn get-user-from-creds [username password]
